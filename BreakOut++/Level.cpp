...
Level::Level(int level, PlaySideBar* sideBar,Player* player,Ball* ball) {
	...
	mReadyLabel = new Texture("READY", "emulogic.ttf", 24, { 200,75,75});
...
void Level::ParseXml() {
	...
	if (exists_test(fullpath)) {
		...
		mLevelTexture = new Texture(mBackgroundTexture);
		mLevelTexture->Parent(this);
		mLevelTexture->Pos(Vector2(Graphics::Instance()->SCREEN_WIDTH * 0.63f-22.0f, Graphics::Instance()->SCREEN_HEIGHT * 0.5f));

		tinyxml2::XMLElement* brickElement = doc.FirstChildElement("Level")->FirstChildElement("BrickTypes");
		for (tinyxml2::XMLElement* child = brickElement->FirstChildElement(); child != nullptr; child = child->NextSiblingElement())
		{
		...
		for (int i = 0; i < (mRowCount * mColumnCount) * 2; i += 2) {
			...
			for (int j = 0; j < mBrickTypes.size(); j++) {
				if (tmp[i] == '-') {
					...
					mBricks[total]->Pos(Vector2((50.0f + (float)mColumnSpacing) * k, (25.0f + (float)mRowSpacing) * h));
					k++;
					total++;
				}
			}
		...
		mDemoOver = new Texture("Demo Over","emulogic.ttf", 32, { 255,0,0 });
		mDemoOver->Parent(this);
		mDemoOver->Pos(Vector2(Graphics::Instance()->SCREEN_WIDTH * 0.62f, Graphics::Instance()->SCREEN_HEIGHT * 0.5f));
		mDemo = false;
	}
