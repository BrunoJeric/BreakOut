...
void PhysicsManager::UnregisterEntity(unsigned long id) {
	bool found = false;
	for (unsigned int i = 0; i < static_cast<unsigned int>(CollisionLayers::MaxLayers) && !found; i++) {
...
PhysicsManager::PhysicsManager() {
	...
	for (unsigned int i = 0; i < static_cast<unsigned int>(CollisionLayers::MaxLayers); i++) {
		...
		mLayerMasks[i]= std::bitset<static_cast<unsigned int>(CollisionLayers::MaxLayers)>(static_cast<unsigned int>(CollisonFlags::None));
